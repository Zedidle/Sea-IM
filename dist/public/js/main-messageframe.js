  ...
  >
    ...
    >
      ...
      <hr style='border-color: #C0D0D0;border-width: 2px;'>
			<div class="info">
				<div><h3>UID&nbsp: </h3><h4>{{moreInfo.uid}}</h4></div>
				<div><h3>名称&nbsp: </h3><h4>{{moreInfo.name}}</h4></div>
        <div v-show='isTeam'><h3>等级&nbsp: </h3><h4>{{moreInfo.level}}</h4></div>
				<div v-show='isTeam'><h3>人数&nbsp: </h3><h4>{{moreInfoTeamMemberNumber}}</h4></div>
				<div v-show='!isTeam'><h3>性别&nbsp: </h3><h4>{{moreInfo.sex}}</h4></div>
				<div v-show='!isTeam'><h3>爱好&nbsp: </h3><h4>{{moreInfo.hobby}}</h4></div>
				<div v-show='!isTeam'><h3>生日&nbsp: </h3><h4>{{moreInfo.birthday}}</h4></div>
				<div><h3>介绍&nbsp: </h3><h4>{{moreInfo.introduce}}</h4></div>
			</div>
